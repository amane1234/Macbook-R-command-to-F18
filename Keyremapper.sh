...
mkdir -p "$BIN_DIR" || { echo " Error, Failed to create directory" >&2; exit 1; }
chmod 755 "$BIN_DIR" || { echo "Error, Failed to set directory permissions" >&2; exit 1; }
...
chmod 755 "$SCRIPT_PATH" || { echo "Error, Failed to make script executable" >&2; exit 1; }
...
mv -f "$PLIST_SOURCE" "$PLIST_TARGET" || { echo "Error, Failed to install launch agent" >&2; exit 1; }
chown root "$PLIST_TARGET" || { echo "Error, Failed to set launch agent ownership" >&2; exit 1; }
...
if launchctl print system/userkeymapping &>/dev/null; then
    ...
    echo "Error, Service failed to load" >&2
    exit 1
fi
